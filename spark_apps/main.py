...
def main():
    ...
    website_df = (spark.read
                  .csv(data_folder + '/website_dataset.csv', header=True, inferSchema=True, sep=';'))
    # website_df.printSchema()
    # website_df.show(5)

    # trim data and make columns with lower characters
    google_df_trimmed = google_df.select(trim(google_df['address']).alias('address'),
                                         ...
                                         trim(google_df['raw_address']).alias('raw_address'),
                                         ...
                                         trim(google_df['zip_code']).alias('zip_code'),
    ...
    facebook_df_trimmed = facebook_df.select(lower(trim(facebook_df['domain'])).alias('domain'),
                                             trim(facebook_df['address']).alias('address'),
                                             ...
                                             trim(facebook_df['zip_code']).alias('zip_code'))
    ...
    website_df_trimmed = website_df.select(lower(trim(website_df['root_domain'])).alias('root_domain'),
                                           ...
                                           trim(website_df['s_category']).alias('s_category'))
    ...
    (google_df_trimmed_cleared.alias('a').join(facebook_df_trimmed.alias('b'),
     ...
     .select(col('b.link').alias('link'),
             col('b.page_type').alias('page_type'),
             col('b.email').alias('page_type'),
             col('a.raw_phone').alias('raw_phone'),
             col('c.language').alias('language'),
             col('c.legal_name').alias('legal_name'),
             ...
             .otherwise(col('b.name')).alias('name'))
